...
func TraceImage(image string) (*MetadataTrace, error) {
	...
	case toolAvailable("docker"):
		...
		extraObservations = append(extraObservations, Observation{
			...
			Hint:    "docker inspect usually exposes config labels but not remote manifest annotations; install crane or skopeo for better metadata tracing",
		})
	default:
	...
	trace.Observations = BuildObservations(trace.ManifestAnnotations, trace.ConfigLabels)
...
func BuildObservations(annotations map[string]string, labels map[string]string) []Observation {
...
func traceWithCrane(trace *MetadataTrace, image string) error {
	...
	trace.ManifestAnnotations = ParseManifestAnnotations(manifest)
